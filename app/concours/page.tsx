...
export default function Page() {
	...
	const {data, status, refetch} = useQuery(
		'elementsArticlesConcours', async(context) => {
			const query = `{"total": count(*[_type=="articles" && category->slug.current=='concours']), "articles" : *[_type=="articles" && category->slug.current=='concours' && hidePublication != true]{_id}|order(_createdAt desc)[${page*itemsPerPage}...${page*itemsPerPage + itemsPerPage+1}]}`;
