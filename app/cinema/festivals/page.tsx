...
export default function Page() {
	...
	const {data, status, refetch} = useQuery(
		'elementsArticlesCinemaFestival', async(context) => {
			const query = `{"total": count(*[_type=="articles" && category->slug.current=='cinema' && subcategory->slug.current=='festival']), "articles" : *[_type=="articles" && category->slug.current=='cinema' && subcategory->slug.current=='festival']{_id}|order(_createdAt desc)[${page*itemsPerPage}...${page*itemsPerPage + itemsPerPage+1}]}`;
